...
class CartController extends Controller
{
    function postTest(Request $request){
        ...
        if(!Auth::guest()){
            ...
            if(!Auth::guest()){
                $data=Cart::select("products_id","amount","article.name","article.price","category.id","category.macrocategory as mcat","article.sale")->join("article","cart_content.products_id","=","article.id")->join("category","article.cat_id","=","category.macrocategory")->where("cart_content.user_id","=",Auth::user()->id)->get();//->where("cat_id",$category)->where("macrocategory",$macro)->paginate(16);
                $total=0;
