...
class FrontendController extends Controller
{
    ...
    public function searchProduct(Request $request){
        $query=Article::select("article.id","article.name","article.description","article.price","article.imgURI","category.name as nomecat","category.name as nomecat","category.id as idcat","category.macrocategory","article.sale","article.price")->join("category","article.cat_id",'=',"category.id")->where("article.name","like","%".$request->searchbar."%")->orderBy("price","ASC")->paginate(16);
        if($query->isEmpty()){
            return(abort(404));
        }
